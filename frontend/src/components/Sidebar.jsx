...
export default function Sidebar({
  ...
  // "Rankings" opens the leaderboard modal (state lives here so the modal
  // renders as a portal-less overlay above everything, sidebar included).
    ...
    <>
      ...
      <aside className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-footer">
          <ThemeToggle />
          <button
            className="theme-toggle rankings-link" // reuses theme-toggle styling
            onClick={() => setShowRankings(true)}
            aria-label="Show council rankings"
          >
            <span className="theme-toggle-icon">
              ...
              >
                ...
                <path d="M7 6H4a3 3 0 0 0 3 5M17 6h3a3 3 0 1 1-3 5" />
              </svg>
            </span>
            <span className="theme-toggle-label">Rankings</span>
          </button>
        </div>
      </aside>

      {showRankings && (
        <RankingsModal top={4} onClose={() => setShowRankings(false)} />
      )}
    </>
  );
}
